...
func (db *StreamDatabase) Update(oldStreamName, newStreamName, newUrl, newStreamIconPath string) {
	fv, err := db.GetFavouritesByItemName(oldStreamName)
	id := fv.Id
	fmt.Println(fv.StreamName.String)
	if err == nil {
		db.Exec("UPDATE favourites SET StreamName = ?, Url = ?, LogoImage = ? WHERE Id = ?", 
				newStreamName, newUrl, newStreamIconPath, id)
	} else {
		log.Fatal(err)
		item, err := db.GetStreamByItemName(oldStreamName)
		id := item.Id
		if err == nil {
			db.Exec("UPDATE radiometadata SET StreamName = ?, Url = ?, LogoImage = ? WHERE Id = ?", 
					newStreamName, newUrl, newStreamIconPath, id)
		} else {
			log.Fatal(err)
		}
